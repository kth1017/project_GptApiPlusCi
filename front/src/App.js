        ...
        <DialogTitle>도움말</DialogTitle>
            <DialogContent>
                <DialogContentText>
                    ...
                    그렇기 때문에 하고 싶은 질문이 한글이라면 아래의 전체 과정을 진행하는 것을 권장합니다.(하고 싶은 질문이 영어거나 한영 혼합이라면 바로 3번을 진행해주세요.)<br />
                    - Gpt ai 특성상 답변이 정확하다고 확신할 수 없습니다. 교차 검증은 필수!<br /><br />

                    - Gpt api 제공 서버에 이상이 있거나 ai가 답변하지 못할 질문을 할 경우 긴 로딩이 발생하며
                    15초가 지나도 gpt api의 응답이 없을 경우 질문이 취소됩니다.<br /><br />

                    1. 번역이 가능한 한글 질문을 하실 예정이시라면 아래 그림처럼 질문을 작성한 뒤
                    번역 버튼을 눌러주세요.(번역은 한 > 영만 가능합니다.) 질문은 100자까지 허용합니다.<br /><br />
                    * 하고 싶은 질문 자체가 한글와 영어가 혼용되있다면 바로 3번으로 넘어가주세요.<br /><br />
                    <img src={require("./assets/pictures/번역.png")} /><br /><br />
                    2. 궁금하신 질문이 떠오르시지 않는다면 아래 추천 질문 버튼을 통해 질문 문장을
                    완성하실 수 있습니다. 먼저 아래처럼 카테고리를 선택하세요.<br /><br />
                    <img src={require("./assets/pictures/그림1.png")} /><br /><br />
                    카테고리를 선택하면 아래처럼 상세 질문 버튼 리스트가 출력됩니다.<br /><br />
                    <img src={require("./assets/pictures/그림3.png")} /><br />
                    상세 질문 버튼 중 하나를 클릭하시면 다음 탭에서 질문이 자동 완성됩니다.<br /><br />
                    3. 앞서 번역했거나 추천된 질문이 아래처럼 출력되거나<br />
                    직접 ai에게 할 질문을 입력하신 뒤 버튼을 눌러주세요.<br /><br />
                    ...
                    혹시 로딩 스피너가 사라지지 않는다면 정상 요청을 한 번 다시 해주세요.<br /><br />
                    <img src={require("./assets/pictures/그림5.png")} /><br /><br />
                    5. 답변이 출력되었을 때 한글로 번역이 필요한 경우 번역 버튼을 눌러주세요.<br /><br />
...
function Box() {
    ...
    return <>
        <Container sx={{ m:1 , border: 1, padding: 2, borderColor: 'divider', backgroundColor: 'rgba(240, 255, 255, 0.8)'}}>
            <h3>번역할 질문 입력(필요시 입력)</h3> <Form></Form>
            <h3>추천 질문 분류 선택(필요시 선택)</h3>
    ...
    return <form onSubmit={event =>{
        ...
        axios.post('/api/requestTransKE',
                    ...
                    console.log(response.data);
                      // 들어오는 json이 배열  
                    setTransQ(JSON.stringify(response.data.translations[0].text));
                })
